# Contributing to Milpa Data

Thanks for your interest in contributing! Milpa Data is the runtime-native persistence
primitive of the Milpa framework — plain entities (no ORM base class, no attributes) behind
a small repository contract, with two interchangeable backends, file (JSON) and in-memory,
sitting behind one `RepositoryInterface`. It has **zero package dependencies** beyond PHP
itself.
...
- **Respect the tier boundary.** Milpa Data is a zero-dependency leaf primitive —
  the persistence contract; backends slot in behind it. It depends on nothing but
  PHP itself: no `milpa/core`, no ORM, no serialization library. `FileRepository`
  talks to a flat JSON file directly (`json_decode`/`json_encode`/
  `file_put_contents`); `InMemoryRepository` keeps an in-process array. A change
  that would require adding a package dependency belongs in a different package,
  not here.
...
`milpa/event-store`, `milpa/data`) shares one coding standard, committed verbatim in
